...
var viewProducts = function(){
	...
	connection.query('SELECT * FROM Products', function(err, res){
		...
		for(var i=0; i<res.length; i++){
			table.push(
				[res[i].id, res[i].name, res[i].department, res[i].price, res[i].quantity]
			)
		}
...
var addInventory = function(){
	...
	prompt.get(addInvt, function(err, res){
		...
		connection.query("UPDATE Products SET quantity = (quantity + ?) WHERE id = ?;", [inventoryUpdate[0].inventoryAmount, inventoryUpdate[0].inventoryID], function(err, result){
			...
			connection.query("SELECT * FROM Products WHERE id = ?", inventoryUpdate[0].inventoryID, function(error, resOne){
				console.log('')
				console.log('The new updated stock quantity for id# '+inventoryUpdate[0].inventoryID+ ' is ' + resOne[0].quantity)
				console.log('')
				connection.end()
			})
...
var addNewProduct = function(){
	...
	prompt.get(newProduct, function(err, res){
		...
		connection.query('INSERT INTO Products (id, name, department, price, quantity) VALUES (?, ?, ?, ?, ?);', [addedProduct[0].newIdNum, addedProduct[0].newItemName, addedProduct[0].newItemDepartment, addedProduct[0].newItemPrice, addedProduct[0].newStockQuantity], function(err, result){
