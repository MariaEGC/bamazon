...
var viewProductSales = function(){
	...
	connection.query('SELECT * FROM product_sale', function(err, res){
		if(err) console.log('Error: ' + err)

		//this loops through the data pulled from the totalprofits database and pushes it into the table above
		for(var i = 0; i<res.length; i++){
			table.push(
				[res[i].id, res[i].department, res[i].over_head_costs, res[i].total_product, res[i].product_sale]
				)
		}
...
var createDepartment = function(){
	...
	prompt.get(newDepartment, function(err, res){
		...
		connection.query('INSERT INTO Departments (department, over_head_costs, total_product) VALUES (?, ?, ?);', [newDept[0].deptName, newDept[0].overHeadNew, newDept[0].autoTotalSales], function(err, result){
			if(err){
